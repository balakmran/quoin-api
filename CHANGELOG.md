...
- **Docs**: the release skill, release workflow, AI-setup guide and
  `CLAUDE.md` no longer point a generated project at CI jobs or a
  `copier.yml` it does not have. The template-only fences gained a
  `%% ...` form so a Mermaid node can carry one. Update-safe.
...
- **CI**: a **Scaffold Smoke Test** workflow generates a project from
  ...
  project.
- **Docs**: a new [Staying Current](docs/guides/staying-current.md)
...
- **Release tooling**: `just tag` (alias `just release`) now publishes
  ...
  **Manual reconciliation**: this is a breaking change to a covered CLI
  ...
  policy](docs/guides/api-stability.md#breaking-vs-non-breaking) -- it
...
- **API**: every list endpoint now returns a standard `Page[T]`
  ...
  [Pagination guide](docs/guides/pagination.md).
- **API**: a `deprecated()` dependency (`app/core/versioning.py`) stamps
  ...
  [Deprecating Endpoints guide](docs/guides/deprecating-endpoints.md).
...
- **Users**: `DELETE /users/{id}` is now a soft delete — it stamps a
  ...
  [Soft Delete guide](docs/guides/soft-delete.md).
