...
- **Edge rate limiting** — the template assumes an upstream limiter
  ...
  [Edge rate limiting](../guides/deployment.md#edge-rate-limiting).
- **TLS termination** and forwarded-header trust — see
  [Behind a load balancer or reverse proxy](../guides/deployment.md#behind-a-load-balancer-or-reverse-proxy).
...
- [Authentication guide](../guides/authentication.md) — OIDC/JWT
